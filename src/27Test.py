...
class TestFourBarLinkageCases(unittest.TestCase):
    ...
    def check_motion_case(self, T1, T2, T3, T1T2, T1T3, expected_input, expected_output):
        ...
        # Check classification of input link a
        if T1T2 >= 0 and T3 >= 0:
            input_link = "crank"
        elif T1T2 < 0 and T3 < 0:
            input_link = "rocker"
        elif T1T2 >= 0 and T3 < 0:
            input_link = "0–rocker"
        elif T1T2 < 0 and T3 >= 0:
        ...
        # Check classification of output link b
        if T2 > 0 and T1T3 > 0:
            output_link = "rocker"
        elif T2 <= 0 and T1T3 > 0:
            output_link = "0–rocker"
        elif T2 > 0 and T1T3 <= 0:
            output_link = "π–rocker"
        elif T2 <= 0 and T1T3 <= 0:
            output_link = "crank"

        # Assert input and output classifications
        self.assertEqual(input_link, expected_input)
        self.assertEqual(output_link, expected_output)
